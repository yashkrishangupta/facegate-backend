...
const SELECT_SUBJECT = `
    SELECT s.subject_id, s.department_id, d.department_name, s.subject_code,
           s.subject_name, s.program, s.semester, s.credits, s.subject_type,
...
export const getAllSubjects = async (filters: { program?: string; semester?: string; department_id?: string }) => {
    const conditions = ["s.is_active = TRUE"];
    const values: any[] = [];
    if (filters.program) { values.push(filters.program); conditions.push(`s.program = $${values.length}`); }
...
export const createSubject = async (data: any) => {
    const {
        department_id, subject_code, subject_name, program, semester,
